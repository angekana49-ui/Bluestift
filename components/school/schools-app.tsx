    ...
    <AppShell theme={t}>
          ...
          {collapsed && (
            <div style={{ display: "flex", justifyContent: "center" }}>
              <span
                ...
                style={{ width: 32, height: 32, borderRadius: "50%", background: t.sidebarActiveBg, color: t.sidebarText, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 13, fontWeight: 700 }}
              >
                {role === "admin" ? "A" : "P"}
              </span>
            </div>
      ...
      <MainCard theme={t} column minWidth={340}>
        <div style={{ padding: "18px 26px", display: "flex", alignItems: "center", borderBottom: `1px solid ${t.cardBorder}` }}>
          <span style={{ fontSize: 18, fontWeight: 800, fontFamily: display, color: t.text }}>{TITLES[view]}</span>
          <span style={{ marginLeft: "auto", fontSize: 14, border: `1px solid ${t.inputBorder}`, background: t.inputBg, color: t.mutedLight, borderRadius: 99, padding: "8px 16px" }}>Rechercher élèves, classes...</span>
  ...
  return (
    <div style={{ border: `1px solid ${t.cardBorder}`, borderRadius: 20, padding: 18, background: t.cardBg2 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
        <span style={{ fontSize: 16, fontWeight: 700, color: t.text }}>Sessions vs. maîtrise</span>
      ...
      <div style={{ textAlign: "center", marginTop: 8 }}>
        <span style={{ background: t.ctaBg, color: t.ctaText, borderRadius: 99, padding: "6px 12px", fontSize: 13 }}>Juin 2026 — Maîtrise en hausse</span>
      </div>
    </div>
  );
}
...
function OverviewProf({ theme: t }: { theme: AppTheme }) {
  return (
    <div style={{ padding: "24px 26px" }}>
      <div style={{ fontSize: 15, color: t.muted, marginBottom: 18 }}>Tes classes et comment tes élèves progressent.</div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(130px,1fr))", gap: 12, marginBottom: 18 }}>
      ...
      <div style={{ border: `1px solid ${t.cardBorder}`, borderRadius: 20, padding: 18, background: t.cardBg2 }}>
        <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 14, color: t.text }}>Mes classes</div>
    ...
    <div style={{ padding: "24px 26px" }}>
      <div style={{ border: `1px solid ${t.cardBorder}`, borderRadius: 20, padding: 18, background: t.cardBg2 }}>
        ...
        <div style={{ marginTop: 14 }}>
          <span style={{ fontSize: 14, background: t.ctaBg, color: t.ctaText, borderRadius: 99, padding: "8px 14px", fontWeight: 600 }}>+ Ajouter une classe</span>
        </div>
      </div>
    </div>
  );
...
function TableHead({ theme: t, cols, labels }: { theme: AppTheme; cols: string; labels: string[] }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: cols, fontSize: 13, color: t.mutedLight, padding: "0 4px 10px" }}>
      {labels.map((l) => (
        <span key={l}>{l}</span>
      ))}
    </div>
...
function ClassRow({ theme: t, cols, cells, pct }: { theme: AppTheme; cols: string; cells: string[]; pct: number }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: cols, alignItems: "center", padding: "12px 4px", borderTop: `1px solid ${t.cardBorder}` }}>
      <span style={{ fontSize: 15, fontWeight: 600, color: t.text }}>{cells[0]}</span>
      {cells.slice(1).map((c, i) => (
        <span key={i} style={{ fontSize: 14, color: t.muted }}>{c}</span>
      ))}
      <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <ProgressBar theme={t} pct={pct} width={70} />
        <span style={{ fontSize: 14, color: t.text }}>{pct}%</span>
      </span>
    </div>
  );
}
...
function KernelView({ theme: t }: { theme: AppTheme }) {
  return (
    <div style={{ padding: "24px 26px", display: "grid", gridTemplateColumns: "1fr", gap: 18 }}>
      <div style={{ border: `1px solid ${t.cardBorder}`, borderRadius: 20, padding: 18, background: t.cardBg2 }}>
        <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 16, color: t.text }}>Ajustements du kernel</div>
        ...
        <div style={{ marginTop: 18, display: "flex", gap: 10, alignItems: "center" }}>
          <span style={{ fontSize: 14, background: t.ctaBg, color: t.ctaText, borderRadius: 99, padding: "9px 16px", fontWeight: 600 }}>Lancer une simulation</span>
          <span style={{ fontSize: 14, color: t.mutedLight }}>Impact estimé : +6pts / 4 sem.</span>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ padding: "24px 26px", display: "grid", gridTemplateColumns: "1fr", gap: 18 }}>
      <div style={{ border: `1px solid ${t.cardBorder}`, borderRadius: 20, padding: 18, background: t.cardBg2 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
          ...
          <span style={{ fontSize: 16, fontWeight: 700, color: t.text }}>Activité Raya — établissement</span>
        </div>
...
function RayaRow({ theme: t, cls, sessions, tone }: { theme: AppTheme; cls: string; sessions: string; tone: string }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr 1fr", alignItems: "center", padding: "12px 4px", borderTop: `1px solid ${t.cardBorder}`, fontSize: 14 }}>
...
function SettingsView({ theme: t, dark, onToggleDark }: { theme: AppTheme; dark: boolean; onToggleDark: () => void }) {
  return (
    <div style={{ padding: "24px 26px" }}>
      ...
      <SettingsCard theme={t}>
        <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 16, color: t.text }}>École</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <span style={{ marginTop: 6, alignSelf: "flex-start", fontSize: 14, color: status.danger, cursor: "pointer" }}>Gérer les accès administrateurs</span>
        </div>
      </SettingsCard>
      ...
      <SettingsCard theme={t} mt>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: t.rowActiveBg, borderRadius: 12, padding: "14px 16px", marginBottom: 14 }}>
          <div>
            <div style={{ fontSize: 15, fontWeight: 700, color: t.text }}>Plan Classroom — 29€/mois</div>
            <div style={{ fontSize: 13, color: t.muted, marginTop: 2 }}>Prochain prélèvement le 1er août 2026</div>
          </div>
          <span style={{ fontSize: 13, background: t.ctaBg, color: t.ctaText, borderRadius: 99, padding: "8px 14px", fontWeight: 600, whiteSpace: "nowrap" }}>Passer à School</span>
        </div>
        ...
        <div style={{ fontSize: 13, color: t.muted, marginBottom: 8 }}>Factures</div>
...
function InvoiceRow({ theme: t, label, amount }: { theme: AppTheme; label: string; amount: string }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", padding: "9px 4px", borderTop: `1px solid ${t.cardBorder}`, fontSize: 14, color: t.text }}>
      <span>{label}</span>
    ...
    <RightPanel theme={t} width={300}>
      <div>
        ...
        <div style={{ background: t.rowActiveBg, borderRadius: 12, padding: 11, marginBottom: 6 }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: t.text }}>6 élèves de 5e-B décrochent</div>
          <div style={{ fontSize: 13, color: t.muted, marginTop: 2 }}>Il y a 2h</div>
        </div>
        <div style={{ background: t.rowActiveBg, borderRadius: 12, padding: 11 }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: t.text }}>M. Fotso : 3 classes sans session depuis 5j</div>
          <div style={{ fontSize: 13, color: t.muted, marginTop: 2 }}>Hier</div>
        </div>
      </div>
      ...
      {isOverviewAdmin && (
        <>
          <div style={{ border: `1px solid ${t.cardBorder}`, borderRadius: 20, padding: 16, background: t.cardBg2 }}>
            <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 10, color: t.text }}>Kernel — maîtrise globale</div>
            <MasteryGauge theme={t} valueLabel="1,204" caption="élèves suivis" dashoffset={50} />
          </div>
          <div style={{ border: `1px solid ${t.cardBorder}`, borderRadius: 20, padding: 16, background: t.cardBg2 }}>
            <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 10, color: t.text }}>Insights Raya</div>
      ...
      {view === "raya" && (
        <>
          <div style={{ background: t.ctaBg, color: t.ctaText, borderRadius: 20, padding: 16 }}>
            <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 6 }}>Personnalité de Raya</div>
            <div style={{ fontSize: 13, opacity: 0.7, marginBottom: 12 }}>Ton par défaut pour toutes les classes</div>
            <span style={{ display: "inline-block", background: "rgba(255,255,255,0.15)", borderRadius: 99, padding: "8px 14px", fontSize: 13, fontWeight: 600 }}>Configurer</span>
          </div>
          <div style={{ border: `1px solid ${t.cardBorder}`, borderRadius: 20, padding: 16, background: t.cardBg2 }}>
            <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 10, color: t.text }}>Contenus signalés</div>
            <div style={{ fontSize: 14, color: t.muted }}>Aucun contenu en attente de revue.</div>
          </div>
        </>
      )}
    </RightPanel>
