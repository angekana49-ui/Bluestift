        ...
        {showRoomsList && (
          <HistoryList>
            ...
            <div style={{ background: t.rowActiveBg, borderRadius: 9, padding: "8px 10px" }}>
              ...
              <div style={{ fontSize: 14, color: t.sidebarMuted, marginTop: 1 }}>4 participants · live</div>
            </div>
...
function ChatView({
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
      ...
      >
        <Avatar initials="AI" size={32} bg={status.aiIndigo} style={{ fontSize: 15 }} />
        ...
        {chatFilesOpen && (
          ...
          >
            <div style={{ fontSize: 14, fontWeight: 700, color: t.text, marginBottom: 8 }}>Documents de la session</div>
        ...
        <div style={{ display: "flex", flex: 1, padding: "28px 32px", flexDirection: "column", gap: 16, overflow: "auto" }}>
          ...
          <Bubble theme={t} maxWidth="62%">
            ...
            <div className="shine" style={{ marginTop: 10, background: t.cardBg2, border: `1px solid ${t.cardBorder}`, borderRadius: 12, padding: 12 }}>
              <div style={{ fontSize: 14, color: t.muted }}>Exercice généré</div>
              <div style={{ fontSize: 16, fontWeight: 600, marginTop: 2, color: t.text }}>1/3 + 1/4 = ?</div>
            </div>
          </Bubble>
        </div>
      )}
  ...
  return (
    ...
    >
      ...
      <div style={{ minWidth: 0, flex: 1 }}>
        ...
        <div style={{ fontSize: 14, color: t.muted }}>{meta}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
      <div style={{ padding: "16px 24px", borderBottom: `1px solid ${t.cardBorder}`, display: "flex", alignItems: "center", gap: 12 }}>
        <span style={{ fontSize: 17, fontWeight: 700, color: t.text, flex: "none" }}>Room · 6e-A Maths</span>
        ...
        <span style={{ marginLeft: "auto", fontSize: 14, color: status.positive, flex: "none" }}>● live</span>
        <IconButton theme={t} onClick={onToggleRight}>
          <IconPanel size={14} />
        </IconButton>
      </div>
      ...
      {roomTab === "group" ? (
        <>
          <div style={{ flex: 1, padding: "26px 30px", display: "flex", flexDirection: "column", gap: 14, overflow: "auto" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
              <Avatar initials="AI" size={26} bg={status.aiIndigo} style={{ fontSize: 14 }} />
              <span style={{ fontSize: 15, fontWeight: 700, color: t.text }}>Raya modère la session</span>
            </div>
            <Bubble theme={t} maxWidth="60%">Léo, tu peux expliquer à Sami comment tu as trouvé le dénominateur commun ?</Bubble>
            <div style={{ maxWidth: "52%", background: t.bubbleAccentBg, color: t.text, borderRadius: "16px 16px 16px 4px", padding: "13px 16px", fontSize: 16, lineHeight: 1.65 }}>
        ...
        <>
          <div style={{ flex: 1, padding: "26px 30px", display: "flex", flexDirection: "column", gap: 14, overflow: "auto" }}>
            <div style={{ fontSize: 14, color: t.mutedLight, textAlign: "center", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
              <IconLock size={12} />
  ...
  return (
    <div style={{ borderTop: `1px solid ${t.cardBorder}`, padding: "14px 24px" }}>
      <div style={{ display: "flex", gap: 10, alignItems: "center", background: t.inputBg, border: `1px solid ${t.inputBorder}`, borderRadius: 99, padding: "7px 7px 7px 18px" }}>
        <span style={{ flex: 1, fontSize: 16, color: t.mutedLight }}>{placeholder}</span>
        <span style={{ width: 34, height: 34, borderRadius: "50%", background: t.ctaBg, color: t.ctaText, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>↑</span>
      </div>
    </div>
  );
}
...
function ToolsView({ theme: t }: { theme: AppTheme }) {
  ...
  return (
    <div style={{ flex: 1, padding: "36px 44px", overflow: "auto" }}>
      ...
      <div style={{ fontSize: 16, color: t.muted, marginBottom: 26 }}>Génère des quiz, résumés et flashcards depuis n&apos;importe quel cours.</div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16, maxWidth: 900 }}>
        {cards.map((c) => (
          <div key={c.title} style={{ background: t.cardBg2, border: `1px solid ${t.cardBorder}`, borderRadius: 18, padding: 20 }}>
            ...
            <div style={{ fontSize: 17, fontWeight: 700, color: t.text }}>{c.title}</div>
            <div style={{ fontSize: 15, color: t.muted, marginTop: 6, lineHeight: 1.6 }}>{c.desc}</div>
          </div>
        ))}
      </div>
      <div style={{ marginTop: 20, border: `1px dashed ${t.cardBorder}`, borderRadius: 18, padding: 22, maxWidth: 900, textAlign: "center", color: t.mutedLight, fontSize: 15 }}>
...
function KernelView({ theme: t }: { theme: AppTheme }) {
  return (
    <div style={{ flex: 1, padding: "36px 44px", overflow: "auto" }}>
      ...
      <div style={{ fontSize: 16, color: t.muted, marginBottom: 26 }}>Ta maîtrise, concept par concept — pas une note globale.</div>
      <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 20, maxWidth: 1100 }}>
        <div style={{ background: t.cardBg2, border: `1px solid ${t.cardBorder}`, borderRadius: 20, padding: 20 }}>
          <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 10, color: t.text }}>Maîtrise globale</div>
          <MasteryGauge theme={t} valueLabel="79%" caption="tous sujets" dashoffset={60} />
        </div>
        <div style={{ background: t.cardBg2, border: `1px solid ${t.cardBorder}`, borderRadius: 20, padding: 20 }}>
          <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 14, color: t.text }}>Par concept</div>
...
function ConceptLine({ theme: t, label, pct, color }: { theme: AppTheme; label: string; pct: number; color: string }) {
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 15, marginBottom: 6, color: t.text }}>
        <span>{label}</span>
    ...
    <div style={{ flex: 1, padding: "36px 44px", overflow: "auto" }}>
      ...
      <SettingsCard theme={t}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div style={{ fontSize: 17, fontWeight: 700, color: t.text }}>Thème</div>
            <div style={{ fontSize: 14, color: t.muted, marginTop: 2 }}>Basculer entre mode clair et sombre</div>
          </div>
      ...
      <SettingsCard theme={t}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 20 }}>
          ...
          <div>
            <div style={{ fontSize: 17, fontWeight: 700, color: t.text }}>Emma M.</div>
            <div style={{ fontSize: 15, color: t.muted }}>6e-A · Lycée Voltaire</div>
          </div>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <span style={{ marginTop: 6, alignSelf: "flex-start", fontSize: 15, color: status.danger, cursor: "pointer" }}>Se déconnecter</span>
        </div>
      </SettingsCard>

      <SettingsCard theme={t} mt>
        <div style={{ fontSize: 17, fontWeight: 700, marginBottom: 16, color: t.text }}>Facturation</div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: t.rowActiveBg, borderRadius: 12, padding: "14px 16px", marginBottom: 14 }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 700, color: t.text }}>Plan Étudiant — Gratuit</div>
            {/* Mirrors RAYA_ENTITLEMENTS.free.messagesPerDay. Hardcoded because
                ...
                tier got a chat cap. */}
            <div style={{ fontSize: 14, color: t.muted, marginTop: 2 }}>30 messages Raya par jour</div>
          </div>
          <span style={{ fontSize: 14, background: t.ctaBg, color: t.ctaText, borderRadius: 99, padding: "8px 14px", fontWeight: 600, whiteSpace: "nowrap" }}>Passer à Classroom</span>
        </div>
        <div style={{ fontSize: 14, color: t.muted, marginBottom: 8 }}>Moyen de paiement</div>
        <div style={{ border: `1px solid ${t.inputBorder}`, background: t.inputBg, borderRadius: 10, padding: "10px 14px", fontSize: 16, color: t.text, marginBottom: 14 }}>Aucun moyen de paiement enregistré</div>
        <div style={{ fontSize: 14, color: t.muted, marginBottom: 8 }}>Historique</div>
        <div style={{ fontSize: 15, color: t.mutedLight }}>Aucune facture — le plan gratuit ne génère pas de facturation.</div>
      </SettingsCard>
    </div>
  );
}
...
export function Field({ theme: t, label, value, letterSpacing }: { theme: AppTheme; label: string; value: string; letterSpacing?: string }) {
  return (
    <div>
      ...
      <div style={{ border: `1px solid ${t.inputBorder}`, background: t.inputBg, borderRadius: 10, padding: "10px 14px", fontSize: 16, color: t.text, letterSpacing }}>{value}</div>
    </div>
  );
}
...
function ChatRightPanel({ theme: t, onGoTools }: { theme: AppTheme; onGoTools: () => void }) {
  return (
    <RightPanel theme={t} width={270}>
      <div>
        <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 10, color: t.text }}>Notifications</div>
      ...
      <div style={{ border: `1px solid ${t.cardBorder}`, borderRadius: 16, padding: 16 }}>
        <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 8, color: t.text }}>Maîtrise — Fractions</div>
        <MasteryGauge
          theme={t}
          valueLabel="61%"
      ...
      <div style={{ border: `1px solid ${t.cardBorder}`, borderRadius: 16, padding: 16 }}>
        <div style={{ background: t.cardBg2, borderRadius: 12, padding: 11, marginBottom: 10 }}>
          <div style={{ fontSize: 14, color: t.muted }}>Encore bloqué·e ?</div>
          <div style={{ fontSize: 15, fontWeight: 700, marginTop: 3, color: t.text }}>Raya propose 3 exercices ciblés</div>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          <div onClick={onGoTools} style={{ cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", background: t.ctaBg, color: t.ctaText, borderRadius: 10, padding: "9px 12px", fontSize: 15, fontWeight: 600 }}>
            <span>Flashcards</span>
            <span>→</span>
          </div>
          <div onClick={onGoTools} style={{ cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", background: t.cardBg2, color: t.text, border: `1px solid ${t.cardBorder}`, borderRadius: 10, padding: "9px 12px", fontSize: 15, fontWeight: 600 }}>
...
function NotifCard({ theme: t, title, meta, mb }: { theme: AppTheme; title: string; meta: string; mb?: boolean }) {
  return (
    <div style={{ background: t.rowActiveBg, borderRadius: 12, padding: 10, marginBottom: mb ? 6 : 0 }}>
      ...
      <div style={{ fontSize: 14, color: t.muted, marginTop: 2 }}>{meta}</div>
    </div>
  );
}
...
function RoomsRightPanel({ theme: t }: { theme: AppTheme }) {
  return (
    <RightPanel theme={t} width={250} padding={16}>
      ...
      <div>
        <div style={{ fontSize: 14, color: t.muted, marginBottom: 10 }}>Fichiers partagés</div>
      ...
      <div>
        <div style={{ fontSize: 14, color: t.muted, marginBottom: 10 }}>Participants (4)</div>
        ...
        <div style={{ marginTop: 16, fontSize: 15, background: t.cardBg2, color: t.text, borderRadius: 99, padding: "9px 12px", textAlign: "center", fontWeight: 600 }}>+ Inviter</div>
      </div>
    </RightPanel>
  );
}
...
function Participant({ theme: t, online, avatar, avatarBg, name, badge, offlineLabel }: { theme: AppTheme; online?: boolean; avatar: string; avatarBg: string; name: string; badge?: string; offlineLabel?: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <span style={{ width: 26, height: 26, borderRadius: "50%", background: avatarBg, fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700 }}>{avatar}</span>
      <span style={{ fontSize: 16, color: t.text }}>{name}</span>
      {badge && <span style={{ marginLeft: "auto", fontSize: 14, background: "#fee2e2", color: "#dc2626", borderRadius: 99, padding: "2px 8px", fontWeight: 600 }}>{badge}</span>}
      {offlineLabel && <span style={{ marginLeft: "auto", fontSize: 14, color: t.mutedLight }}>{offlineLabel}</span>}
    </div>
  );
}
