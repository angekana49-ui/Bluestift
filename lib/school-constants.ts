/**
 * Pure, isomorphic school constants/helpers — NO "server-only" here, so both the
 * server data layer (`lib/school-admin.ts`) and client components
 * (`components/school-admin.tsx`) can import them. (`school-admin.ts` re-exports
 * these, so server-side call sites can keep importing from it.)
 */

/** Extra seats allowed above a class's effectif — hard cap is expectedSize + this. */
export const CLASS_OVERFLOW = 5;

/** Allowed `schools.school_type` values (DB CHECK; not in generated types). */
export const SCHOOL_TYPES = ["primary", "secondary", "university", "other"] as const;

/**
 * ISO 3166-1 alpha-2 countries `{ code, name }`, English names, sorted. The DB
 * stores the 2-letter `code` (schools.country_code); the UI shows the `name` so
 * users pick their country instead of typing an ISO code they may not know.
 */
...
/** Country name for an ISO code, or the code itself if unknown. */
...
/**
 * Academic-year record for a given start year, e.g. 2026 → "2026–2027",
 * spanning Aug 1 (startYear) → Jul 31 (startYear+1). School years run Aug–Aug
 * (the Northern-Hemisphere norm; a country-aware calendar could refine this
 * later since each school stores its country).
 */
export function academicYear(startYear: number) {
  return {
...
/**
 * The academic year spanning `now`, with a strict August cutover: from August 1
 * the current year is the one starting that August; before that it's the prior
 * one. (getMonth() is 0-indexed, so 7 = August.)
 */
export function currentAcademicYear(now = new Date()) {
  const y = now.getFullYear();
  return academicYear(now.getMonth() >= 7 ? y : y - 1);
}

/** Hard cap for a class, or null when no effectif (expected_size) was set. */
export function classCapacity(
  expectedSize: number | null,
  maxOverflow: number | null,
): number | null {
  if (expectedSize == null) return null;
  return expectedSize + (maxOverflow ?? CLASS_OVERFLOW);
}
