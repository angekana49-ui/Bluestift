...
import type { MessageVars } from "@/lib/i18n";
...
export function footerLine(
  brand: DocBrand,
  audience?: string,
  translate?: (key: "doc.generatedBy" | "doc.generatedByFor", vars: MessageVars) => string,
): string {
  const vars = { name: DOC_BRANDS[brand].name, audience: audience?.trim() ?? "", year: new Date().getFullYear() };
  const key = vars.audience ? "doc.generatedByFor" : "doc.generatedBy";
  if (translate) return translate(key, vars);
  return vars.audience
    ? `Generated by ${vars.name} for ${vars.audience} · © ${vars.year} Bluestift`
    : `Generated by ${vars.name} · © ${vars.year} Bluestift`;
}
