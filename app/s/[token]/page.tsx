    ...
    >
      ...
      >
        <header style={{ display: "flex", alignItems: "center", gap: 10, padding: "16px 22px", borderBottom: "1px solid rgba(15,23,42,0.08)", background: "#f6f9fd" }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 700, color: brand.accent }}>{brand.name}</span>
          <span style={{ marginLeft: "auto", fontSize: 11, color: "#8a97a8" }}>Shared · read-only</span>
        </header>

        <div style={{ padding: "20px 24px 4px" }}>
          {data.title && <h1 style={{ fontSize: 22, fontWeight: 800, color: "#0b1220", margin: 0, lineHeight: 1.25 }}>{data.title}</h1>}
        ...
        <div style={{ padding: "6px 24px 8px", color: "#0b1220" }}>
          {blocks.map((b, i) => {
            if (b.type === "h1") return <h2 key={i} style={{ fontSize: 18, fontWeight: 800, margin: "18px 0 8px" }}>{inline(b.text)}</h2>;
            if (b.type === "h2") return <h3 key={i} style={{ fontSize: 15, fontWeight: 700, color: brand.accent, margin: "16px 0 6px" }}>{inline(b.text)}</h3>;
            if (b.type === "h3") return <h4 key={i} style={{ fontSize: 13.5, fontWeight: 700, margin: "12px 0 5px" }}>{inline(b.text)}</h4>;
            if (b.type === "li")
              return (
                <div key={i} style={{ display: "flex", gap: 8, margin: "4px 0", fontSize: 14, lineHeight: 1.6 }}>
                  <span style={{ color: brand.accent, flex: "none" }}>•</span>
                  <span>{inline(b.text)}</span>
                </div>
              );
            return <p key={i} style={{ fontSize: 14, lineHeight: 1.65, margin: "8px 0" }}>{inline(b.text)}</p>;
          })}
        </div>

        <footer style={{ padding: "14px 24px 18px", marginTop: 8, borderTop: "1px solid rgba(15,23,42,0.08)", fontSize: 11, color: "#8a97a8", display: "flex", gap: 6, flexWrap: "wrap" }}>
