...
_Last updated: 2026-09-10 (baseline below is 2026-08-18; see the "Update" section for the delta). Living summary of what's built, how it works, and what's next._
...
Everything below this section was accurate as of 2026-08-18 and has **not**
been re-verified line by line since — most of it should still hold, a few
specifics are now stale (one is corrected inline below). This section is the
honest delta: what's landed in the roughly three weeks since, condensed from
session memory rather than a fresh full audit. Treat it as a pointer to what
to check, not a replacement for reading the code.

**Correction to §5's design-system description**: the app shell no longer has
the fixed cloud background — `AppShell` was redesigned flat (no clouds/haze),
with 4 responsive tiers, drawers, and a retractable right panel; the chat
column is centered. The rest of §5's design-system description (`tokens.ts`,
`theme.tsx`, the shared component inventory) still holds.

**Design system, extended**: `contentBg` vs `cardBg` and `controlBorder`
tokens, plus a shared `PageBody`/`.chat-col` layout, are now test-enforced —
a layout test fails a component that rolls its own width/background instead
of using the shared ones. An extension of §5's rule, not a replacement.

**Typography**: Inter (body) + IBM Plex Sans (display/nav) replaced Inter
Tight; a `text.*` type scale (base 15) now exists. Chat and nav are fully on
it; size propagation to the rest of the app is partial.

**i18n**: a real message catalogue (`lib/i18n`) covers EN/FR/ES/DE via
`LocaleProvider`/`useTranslate`. App chrome is translated everywhere; page
BODIES (marketing pages, generated documents) are translated piecemeal —
`/legal`'s index page and the artifact archive/delete UI (below) are done,
most others are not yet.

**Email branding**: Resend sends go through `sendBrandedEmail(brand)` — one
verified sending domain, a per-product ("Bluestift" vs "Raya"/"Schools")
`From` display name, rather than one generic sender for every email.

**Network resilience**: `lib/net` gained shared primitives — deadline
budgets, an outbox for offline-queued writes, blob retention — meant to be
reused by any new network code rather than each surface rolling its own.

**Generated artifacts — archive & delete (new)**: every place the product
generates a document someone keeps — Tools outputs (summary/quiz/flashcards/
mind map), self-tests, room challenges, Schools Prepare resources, Schools
Reports — now has an archive/delete menu (`components/ui/artifact-menu.tsx`),
matching the conversation history's own pattern: archiving moves a row out of
the default list into a collapsed "Archived" section
(`components/ui/archived-section.tsx`) rather than just dimming it in place;
delete asks first and says honestly what it removes. Two integrity guards
worth knowing about: a Prepare resource linked to a class, and a challenge
with `scope='assignment'`, can only be archived, never hard-deleted — both
feed `getYearArchive`'s year-end record for the school, and deleting one
would quietly thin that record out from under an admin who reads it later.
New `archived_at` columns on `learning.tool_outputs`, `learning.challenges`,
`schools.teacher_resources`, `schools.reports`.

**Generation grounding fix**: the self-test creator used to pre-fill its
"Goal" field with generic boilerplate that alone satisfied validation, and
never sent the "Name" field to the LLM at all — so typing only a topic name
with no document produced a quiz grounded in nothing. Fixed on both the solo
self-test and room-challenge creators; room challenges also gained an opt-in
"base it on our recent room chat" toggle for when the room's fixed subject
has gone stale relative to the live discussion.

**LaTeX/KaTeX math in generated documents (new)**: Tools summaries and
Schools reports can now carry real mathematical notation instead of broken
ASCII. On screen it's real KaTeX (`lib/katex-render.ts`) everywhere these
documents render — Schools' document viewer, Tools' reader view (which also
picked up **bold** rendering it never had before), and public share links. In
the PDF export, a display formula is rasterized (KaTeX + html2canvas) and
embedded as a real typeset image; an inline formula inside a sentence is
approximated to Unicode instead (jsPDF draws plain text runs, and correctly
word-wrapping a mixed text+image line is out of scope for now).

**Billing UI honesty pass**: the Schools Billing tab's "no plan yet" state
used to read like a nag ("Activate a plan below..."); it now says plainly
that the school is on a free, unlimited pilot with no card required. A
Custom/devis plan's "Activate" button no longer opens the same amount/
reference form every other plan does (there is no fixed price to record a
payment against) — it goes to `/contact` instead, the same rule the public
`/pricing` page already applied to a bespoke school plan.

**Security / compliance / performance audits (2026-09)**: three dated passes
happened that this doc doesn't describe — security (a school role is not a
trusted role; fixed a student-record IDOR and an analytics-token leak),
compliance (API age gate, export coverage, school-linked training opt-in,
webhook data minimisation — owner decision recorded there: under-13s admitted
alone, only adults pay, guardian attestation required), and performance
(removed a redundant auth round trip, halved font preload, fixed a caching
bug that cached any non-empty read instead of a successful one). None of
these are reflected in the sections below; there is no single doc for them
yet beyond session notes — worth a proper write-up.

**Domain split — status unchanged, still not applied.** This file and its
companion `docs/domains.md` (§6 below references it) were both accidentally
deleted in a 2026-09-08 cleanup commit (`chore: untrack design exports...`)
and have just been restored from the commit before it. If either goes
missing again, that commit is where to look first.
