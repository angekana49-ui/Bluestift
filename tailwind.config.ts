...
/**
 * Tailwind is used ONLY by the public marketing site (`components/site/*` + the
 * public route pages) — the signed-in app is entirely inline-styled. Preflight
 * (Tailwind's global base reset) is therefore DISABLED so enabling Tailwind
 * cannot disturb any of the inline-styled connected-app screens; the site's own
 * base rules live scoped under `.site-root` in `app/globals.css`. Utilities are
 * only generated for class names that actually appear in `content`.
 */
const config: Config = {
